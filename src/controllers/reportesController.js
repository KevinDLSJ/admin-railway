...
function venta_periodo(req,res){
    ...
    req.getConnection((err, conn)=>{
        conn.query("SELECT DATE_FORMAT(a.fecha, '%d-%m-%Y') AS fecha ,SUM(b.cantidad*b.precio) AS total FROM pedido a, detalle b WHERE b.folio=a.folio and a.fecha>=date(?) AND a.fecha<=date(?) AND a.id_status=4 GROUP BY fecha",[data.fecha_i,data.fecha_f],(err,rep_periodo)=>{
...
function trabajador_dia(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(fecha, '%d-%m-%Y') AS fecha, corre_emp, folio, correo_clie FROM pedido WHERE id_status=4 AND fecha=date(?) GROUP BY fecha, corre_emp;", [data.fecha], (err, reporte_dia) => {
            if (err) throw err;
...
function trabajador_periodo(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(fecha, '%d-%m-%Y') AS fecha, corre_emp, folio, correo_clie FROM pedido WHERE id_status=4 AND fecha>=date(?) AND fecha<=date(?) GROUP BY fecha, corre_emp ORDER BY fecha,corre_emp", [data.fecha_i, data.fecha_f], (err, reporte_periodo) => {
            if (err) throw err;
...
function detalle_dia(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(b.fecha, '%d-%m-%Y') AS fecha, b.folio, b.correo_clie, a.name FROM detalle a JOIN pedido b ON a.folio = b.folio JOIN product c ON c.id_producto = a.id_producto WHERE b.fecha=date(?) AND b.id_status=4 GROUP BY a.folio, a.name;", [data.fecha], (err, reporte_dia) => {
            if (err) throw err;
            if (reporte_dia.length > 0) {
                //console.log(reporte_periodo);
...
function detalle_periodo(req, res) {
    ...
    req.getConnection((err, conn) => {
        conn.query("SELECT DATE_FORMAT(b.fecha, '%d-%m-%Y') AS fecha, b.folio, b.correo_clie, a.name FROM detalle a JOIN pedido b ON a.folio = b.folio JOIN product c ON c.id_producto = a.id_producto WHERE b.fecha>=date(?) AND b.fecha<=date(?) AND b.id_status=4 GROUP BY a.folio, a.name;", [data.fecha_i, data.fecha_f], (err, reporte_periodo) => {
            if (err) throw err;
            if (reporte_periodo.length > 0) {
                //console.log(reporte_periodo);
